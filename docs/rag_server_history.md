...
| 12 | 이번 변경 | 대화형 초보 코칭 톤 강화 | 너무 툭툭 끊기는 답변과 follow-up 문맥 누락 완화 |
...
- 답변자를 `kind Minecraft coach for a brand-new player`로 정의해 더 따뜻하고 차분한 톤을 유도한다.
- 너무 짧고 퉁명스러운 답변을 피하고, 보통 3-5개의 짧은 문장으로 풀어 설명한다.
- 과정형 답변은 `First`, `Then`, `After that` 같은 순서를 사용해 초보자가 따라 하기 쉽게 만든다.
- 사용자가 헷갈릴 때는 다음 작은 행동과 그 이유를 간단히 설명한다.
- `/chat` 요청에 선택형 `history`를 추가해 `then`, `it`, `this tree` 같은 후속 질문의 문맥을 활용할 수 있게 했다.

기대 효과:

- "what is tools?", "how to get sticks?", "what should I do with this tree?" 같은 초보 질문에 덜 딱딱하게 답한다.
- 대화 중 후속 질문이 들어왔을 때 이전 주제를 더 잘 이어받는다.
- 모르는 사람에게 명령하듯 말하는 느낌보다, 옆에서 천천히 알려주는 느낌에 가까워진다.
...
- 초보자에게 친절한 3-5문장 답변을 우선한다.
- 직접 질문한 범위 밖의 장비 최적화, 인챈트, 드롭률, 전략 팁은 제외한다.
- 3x3 제작법은 슬롯 번호로 설명한다.
- 선택형 대화 히스토리가 있으면 후속 질문의 지시어를 이전 대화에 맞춰 해석한다.
